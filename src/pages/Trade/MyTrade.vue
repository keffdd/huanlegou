  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix">
        <span class="username " v-for="(i,index) in selectWays" :key="i.index" :class="{selected:clickindex ==index}" @click="changeWays(index)">{{i}}</span>
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="goods in shopinfo.detailArrayList" :key="goods.skuId">
          <li>
            <img :src="goods.imgUrl" alt="" style="width:82px;height:82px">
          </li>
          <li>
            <p>{{goods.skuName}}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="trade">
      <div class="price">应付金额:<span>¥{{shopinfo.totalAmount}}</span></div>
      <div class="receiveInfo">
        寄送至:
        <span>{{defaultAddress.fullAddress}}</span>
        收货人<span>{{defaultAddress.consignee}}</span>
        电话:<span>{{defaultAddress.phoneNum}}</span>
      </div>
    </div>
...
import { mapState } from 'vuex';
  export default {
    ...
    computed: {
      ...
      ...mapState({shopinfo:state=>state.trade.shopinfo}),
      // 将来提交订单的地址
      defaultAddress(){
    ...
    methods: {
      ...
      async submitOrder(){
        ...
        let data = {
        consignee: this.defaultAddress.consignee, //付款人的名字
        consigneeTel: this.defaultAddress.phoneNum, //付款人的手机号
        deliveryAddress: this.defaultAddress.fullAddress, //付款人收货地址
