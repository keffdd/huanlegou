  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <input placeholder="确认密码" v-model="password1" name="password1" v-validate="{ required: true, is:password }" :class="{ invalid: errors.has('password1') }"/>
        <!-- <span class="error-msg">错误提示信息</span> -->
        <span class="error-msg">{{ errors.first("password1") }}</span>
      </div>
      <div class="controls">
        <input type="checkbox" v-model="agree" name="agree" v-validate="{ required: true, 'agree':true }" :class="{ invalid: errors.has('agree') }"/>
        <span>同意协议并注册<a href="" style="color:crimson">《欢乐购用户协议》</a></span>
        <!-- <span class="error-msg">错误提示信息</span> -->
        <span class="error-msg">{{ errors.first("agree") }}</span>
      </div>
      <div class="btn">
        <button @click="RegisterOver">完成注册</button>
      </div>
    </div>
        ...
        const success = await this.$validator.validateAll(); //全部表单验证
         if(success){
          ...
          try {
            ...
            await this.$store.dispatch('userRegister',{phone,code,password,password1})
