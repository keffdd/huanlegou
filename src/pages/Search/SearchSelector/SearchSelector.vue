  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li v-for="trademark in trademarkList" :key="trademark.tmId" @click="changTradeMark(trademark)">{{trademark.tmName}}</li>
    ...
    <div class="type-wrap" v-for="attrs in attrsList" :key="attrs.attrId">
      ...
      <div class="fl value">
        <ul class="type-list">
          <li v-for="(attrValue,index) in attrs.attrValueList" :key="index" @click="atrrInfo(attrs,attrValue)">
            <a>{{attrValue}}</a>
          </li>
        </ul>
      </div>
...
import { mapGetters } from 'vuex';
  export default {
    ...
    methods: {
      // 自定义事件，传品牌数据给父组件，search组件
      changTradeMark(trademark){
        this.$emit("trademarkInfo",trademark)
      },
      // 自定义事件，穿点击的平台属性给父亲，search组件
      atrrInfo(attrs,attrValue){
